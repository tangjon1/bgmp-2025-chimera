...
Here we analyze the response of the Dcus variants to their native ligand, fumarate, and a novel ligand, aspartate using a fluorescent assay to sort variants by their response to each ligand. By observing the binding affinity of different variants to both fumarate and aspartate, we hope to characterize crucial parts of the protein sequence which determine function. 

The Plesa Lab created a plasmid library containing mutant sequences of the original DcuS sequence, with each plasmid containing a unique 24 bp barcode sequence used to identify the variants after being sorted based on fluorescent response. They engineered the bacterium in this assay to produce a fluorescent response to the EnVZ intermembrane protein. By fusing the DcuS variants to this EnvZ protein, we are therefore able to analyze the fluorescent response.
