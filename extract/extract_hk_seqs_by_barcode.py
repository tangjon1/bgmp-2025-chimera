...
class BarcodeSeqExtractor:
    ...
    def handle_processed_seqs_result(self, processed_seqs: ProcessedSeqs, output_file: TextIO) -> None:
        '''Update current state and write to output file given the results of a processed record'''
        # Update stats dictionary with provided results
        ...
        if processed_seqs.barcode is not None:
            self.barcode_freq_dict[processed_seqs.barcode] = self.barcode_freq_dict.get(processed_seqs.barcode, 0) + 1
        ...
        with (
            ...
            for i, fastq_seqs in enumerate(get_fastq_seqs(input_file)):
                ...
                if len(futures) >= max_pending:
                    ...
                    for future in done:
                        self.handle_processed_seqs_result(future.result(), output_file_sequences)
